...
def run(multiagent=DEFAULT_MA, output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True):
    ...
    if DEFAULT_ACT == ActionType.DISCRETE_2D_COMPLEX:
        counter = np.zeros(25)
    elif DEFAULT_ACT == ActionType.DISCRETE_2D:
        counter = np.zeros(7)
        ...
        if DEFAULT_OBS == ObservationType.KIN:
            if not multiagent:
                if DEFAULT_ACT == ActionType.DISCRETE_2D_COMPLEX:
                    ...
                    counter[action_to_take] += 1
                    rpm_actual = test_env._getRPMs(action_to_take)
                    logger.log(drone=0,
