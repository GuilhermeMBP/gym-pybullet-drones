...
class BaseRLAviary(BaseAviary):
    ...
    def _getRPMs(self, action_to_take):
        ...
        action_mapping_2d = {
            0: (1 + 0.05, 1.0),  # STRONG LEFT
            1: (1 + 0.025, 1.0), # WEAK LEFT
            2: (1 + 0.05, 1.0),  # UP
            3: (1.0, 1.0),       # HOVER
            4: (1 - 0.05, 1.0),  # DOWN
            5: (1.0, 1 + 0.025), # WEAK RIGHT
            6: (1.0, 1 + 0.05)   # STRONG RIGHT
        }
        
        action_mapping_2d_complex = {
            0: (1 + 0.05, 1.0),  # STRONG LEFT
            1: (1 + 0.025, 1.0), # MED LEFT
            2: (1 + 0.01, 1.0),  # WEAK LEFT
            3: (1 + 0.05, 1 + 0.025), # STRONG UP-LEFT
            4: (1 + 0.025, 1 + 0.01),  # MED UP-LEFT
            5: (1 + 0.01, 1 + 0.005),  # WEAK UP-LEFT
            6: (1 + 0.05, 1 + 0.05), # STRONG UP
            7: (1 + 0.025, 1 + 0.025), # MED UP
            8: (1 + 0.01, 1 + 0.01),  # WEAK UP
            9: (1.0, 1.0),       # HOVER
            10: (1 + 0.025, 1 + 0.05), # STRONG UP-RIGHT
            11: (1 + 0.01, 1 + 0.025), # MED UP-RIGHT
            12: (1 + 0.005, 1 + 0.01), # WEAK UP-RIGHT
            13: (1.0, 1 + 0.05), # STRONG RIGHT
            14: (1.0, 1 + 0.025), # MED RIGHT
            15: (1.0, 1 + 0.01),  # WEAK RIGHT
            16: (1 - 0.05, 1 - 0.05), # STRONG DOWN-RIGHT
            17: (1 - 0.01, 1 - 0.025), # MED DOWN-RIGHT
            18: (1 - 0.005, 1 - 0.01), # WEAK DOWN-RIGHT
            19: (1 - 0.05, 1 - 0.05), # STRONG DOWN
            20: (1 - 0.025, 1 - 0.025), # MED DOWN
            21: (1 - 0.01, 1 - 0.01),  # WEAK DOWN
            22: (1 - 0.05, 1 - 0.025), # STRONG DOWN-LEFT
            23: (1 - 0.025, 1 - 0.01), # MED DOWN-LEFT
            24: (1 - 0.01, 1 - 0.005)  # WEAK DOWN-LEFT
        }
        
        if self.ACT_TYPE == ActionType.DISCRETE_2D:
            if action_to_take in action_mapping_2d:
                strong, weak = action_mapping_2d[action_to_take]
                rpm[0, :2] = np.repeat(self.HOVER_RPM * strong, 2)
                rpm[0, 2:] = np.repeat(self.HOVER_RPM * weak, 2)
            ...
            if action_to_take in action_mapping_2d_complex:
                left_factor, right_factor = action_mapping_2d_complex[action_to_take]
                rpm[0, :2] = np.repeat(self.HOVER_RPM * left_factor, 2)
                rpm[0, 2:] = np.repeat(self.HOVER_RPM * right_factor, 2)
